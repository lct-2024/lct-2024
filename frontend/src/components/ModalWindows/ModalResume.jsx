...
import { createCVData, updateCVData, setShowModal } from '../../store/resumeSlice';
...
const ModalResume = () => {
    ...
    const [salary, setSalary] = useState(cvData?.salary || '');
    const [portfolio, setPortfolio] = useState(cvData?.portfolio || '');
    ...
    const saveCVData = () => {
        ...
        if (cvData.id) {
            ...
            dispatch(createCVData({ token, cvData: updatedCV }));
        }
            ...
            <div className="modal">
                <div className="modal-content">
                    <div className="modal-header">
                        <p>Изменить резюме</p>
                        <svg width="16" height="16" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={closeModal}>
                    ...
                    <div className={style.body}>
                        ...
                        <div>
                            ...
                            <textarea id="about" name="about" value={about} onChange={handleInputChange} />
                        </div>
                        <div>
                            <label htmlFor="experience">Опыт работы:</label>
                            <textarea id="experience" name="experience" value={experience} onChange={handleInputChange} />
                        </div>
                        <div>
                            <label>Навыки:</label>
                            <ul>
                            ...
                            <button className={style.skill} onClick={addSkill}>Добавить навык</button>
                        </div>
                        <div>
                            ...
                            <button className={style.skill} onClick={addEducation}>Добавить образование</button>
                        </div>
                        <div>
                            <label htmlFor="salary">Зарплата:</label>
                            <input type="text" id="salary" name="salary" value={salary} onChange={handleInputChange} />
                        </div>
                        <div>
                            <label htmlFor="portfolio">Портфолио:</label>
                            <textarea id="portfolio" name="portfolio" value={portfolio} onChange={handleInputChange} />
                        </div>
                        <button className={style.save} onClick={saveCVData}>Сохранить</button>
                    </div>
                </div>
            </div>
        )
